...
function createCommonFiles(templateDirectory, data) {
    ...
    try {
        var gitignore = fs.readFileSync(path.join(templateDirectory, '.gitignore.mustache'));
        fs.writeFileSync(path.join(data.dst, data.module, '.gitignore'), gitignore);
    } catch (error) {
        if (error.code !== 'EEXIST') {
            console.error(error);
        }
...
function extractKeywords(keywordsStr) {
    "use strict";
    var keywords = ["node-red-contrib-contribgen", "node-red"];
...
//             }
            ...
            var gitignoreTemplate = fs.readFileSync(path.join(data.template, '/.gitignore.mustache'), 'utf-8');
            var gitignoreSourceCode = mustache.render(gitignoreTemplate, params);
            fs.writeFileSync(path.join(data.dst, data.module, '.gitignore'), gitignoreSourceCode);
